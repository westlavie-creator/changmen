...
`packages/venue-adapter` 整体是**双端共享包**（浏览器 + Node）：server 经 `registry` / `loader` 等基础设施目录反向依赖它。原挂在 `client/` 下是历史遗留命名，已于 2026-09-20 迁入 `packages/`（与 `packages/shared` 补进 workspaces 同为一次 workspace hygiene 变更）。
