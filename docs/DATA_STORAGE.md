...
| `client_matches` | embedded matcher 30s 循环 → `match-composer` composeOnce（生产默认 `MATCHER_WRITER=composer`；`legacy` 才走 matcher finalize） | 浏览器 `Client_GetMatchs`（只读，不改写） | 合并后的**电竞**比赛列表 |
