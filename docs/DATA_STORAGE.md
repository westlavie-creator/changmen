...
| `storage/sport/{mlb,soccer}/match_list.json` | 棒球/足球只读列表缓存（Gamma）；**不进 RDS、不写 `client_matches`** | 否 |
...
| `Client_GetBaseballMatchs` / `Client_GetFootballMatchs` | M+J | 进程内存 + `storage/sport/{mlb,soccer}/`；未命中再打 Gamma | 写 sport JSON only | **不读写** `client_matches` / RDS 赛表 |
