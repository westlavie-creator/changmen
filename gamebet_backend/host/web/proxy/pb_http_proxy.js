...
/**
 * 控制台 PB 下单/余额：Yn.get/post → 本地 Node 代发（凭证来自 platforms.json / PB_* env）。
 * GET|POST /esport/pb/proxy?url=<encoded upstream URL>
 */
  ...
  if (!session) {
    ...
    res.end(
      JSON.stringify({
        ...
        hint: "Set PB_GATEWAY + PB_TOKEN or gamebet_backend/data/esport/platforms.json PB",
      }),
    );
    return true;
  }
