 ...
 *
 * Usage:
 *   node scripts/platforms/ob/fetch_ob_view.js --match 5385124130449945 --stage 1
 *   node scripts/platforms/ob/fetch_ob_view.js "<game/view url>" [token]
 */
...
async function main() {
  ...
  if (!args.matchId) {
    console.error(
      "Usage: node scripts/platforms/ob/fetch_ob_view.js --match ID [--stage N] [--token T]\n" +
        "   or: node scripts/platforms/ob/fetch_ob_view.js <game/view url> [token]"
    );
    process.exit(1);
  }
