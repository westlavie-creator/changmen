...
import { STAKE_USDT_TO_CNY } from "@/collectors/stake/constants";
import { stakeAccountHeaders, stakePluginGraphql } from "@/collectors/stake/pluginApi";
import { getStakeTabIdCached, waitForStakeTabId } from "@/collectors/stake/tabId";
...
async function stakeGraphqlForAccount(
  ...
  if (!tabId) throw new Error("未找到Stake标签页");
...
export const stakeProvider: PlatformProvider = {
  ...
  async betting(account, option) {
    const tabId = await resolveTabId();
    if (!tabId) {
      return new BetResult(account.provider, false, "未找到Stake标签页", option.data);
    }
