...
import { collectPbGet } from "@/collectors/pb/http";
import { pbCollectSessionFromAccount, resolvePbCollectAccount } from "@/collectors/pb/session";
import type { CollectBetDto, CollectMatchDto } from "@/types/collect";
import { PLATFORMS } from "@/shared/platform";
import { parseEuroOddsPayload, pbOddsUrl, pbTeamLogo, slugify } from "@/collectors/pb/core";
import { setPbLineId } from "@/collectors/pb/lineCache";
  ...
  const poll = async () => {
    while (!stopped) {
      ...
      try {
        ...
        const collectAccount = resolvePbCollectAccount();
        if (!collectAccount) {
        ...
        const session = pbCollectSessionFromAccount(collectAccount);
