...
| 15 联调加固 | **完成** | PB lineId 回退、IM gameId 解析、SABA 会话初始化、HG 跟单通知 |
| 平博信用盘 v4 | **第一步完成** | `user/account/login` 本地联调通过；见 [app/docs/CREDIT_PLATE.md](./app/docs/CREDIT_PLATE.md) |

## 平博信用盘（`/app/`）

与主站 `Client_Login` 分离：v4 使用 **A8 账号 + `a123456`**，本地 dev 默认走同源 **`/v4.0/`**（Vite → gamebet_backend），避免 `5174` 跨域直连 `api.a8.to`。

详细说明、环境变量与 Network 验收方式：[app/docs/CREDIT_PLATE.md](./app/docs/CREDIT_PLATE.md)。
...
| UserCollectView 平博 | `src/api/v4.ts` + `CollectConfigPanel.vue` | `enterCreditPlate`；见 [app/docs/CREDIT_PLATE.md](./app/docs/CREDIT_PLATE.md) |
...
行为不一致时，以 `gamebet_frontend/vendor/ui-bundle/index.js` 为准 grep 后 port。
