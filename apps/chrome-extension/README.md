...
与 `apps/web/src/config/gamebetExtension.ts` 中 `GAMEBET_EXTENSION_ID_DEFAULT` 一致。
...
2. 开启「开发者模式」
3. 「加载已解压的扩展程序」→ 选择本目录 `changmen/apps/chrome-extension`
4. 确认扩展 ID 为 `mogfpjihgoghabicofkbcmcidlcoofee`
5. 启动 changmen 前端（`apps/web`），PB / Stake / v4 等会经扩展代发 HTTP
