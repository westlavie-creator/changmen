...
/** 使用 data/esport/platforms.json 中已缓存的 gateway/token（前端采集与上轮 feed 写入） */
...
/**
 * Feed / 采集优先复用 platforms.json 有效会话；试玩 login 失败时仍能拉 index。
 */
...
async function syncTfFromA8() {
  try {
    ...
    store.setPlatform("TF", {
      gateway: a8.gateway,
      token: a8.token,
      betName: a8.betName || store.getPlatform("TF")?.betName || "^获胜者$",
