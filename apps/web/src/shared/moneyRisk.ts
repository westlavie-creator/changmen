...
export function executeMoneyRisk(data: MoneyRiskInput): RiskTag[] {
  ...
  // 充值流水倍数
  {
    ...
    if (times < 1) level = "danger";
    else if (times < 2) level = "info";
    tags.push({
  ...
  // 24H 提现
  {
    ...
    else if (money > 1000 || times >= 3) level = "warning";
    tags.push({
  ...
  // 充提比
  {
    ...
    if (ratio <= 1) level = "info";
    else if (ratio < 2) level = "success";
    else if (ratio < 3) level = "warning";
    else level = "danger";
    tags.push({
  ...
  // 高盈利
  {
    ...
    let level: RiskLevel = "info";
    if (profit > 2000 && profitRate > 0.8) level = "danger";
    else if (profit > 2000 || profitRate > 0.8) level = "warning";
    tags.push({
  ...
  // 拒单率
  {
    ...
    let level: RiskLevel = "info";
    if (max === 0) level = "info";
    else if (max < 0.05) level = "success";
    else if (max < 0.1) level = "warning";
    else level = "danger";
    tags.push({
