...
export const A8_MATCH_MAX_FUTURE_SEC: number = 3600;
export const A8_MATCH_LIST_MAX_FUTURE_SEC: number = A8_MATCH_MAX_FUTURE_SEC;

/** IM 仅推赔率：超过该时长无推送则从列表剔除 */
export const IM_ODDS_ACTIVE_MS: number = 3 * 60 * 60 * 1000;
...
export function parseVenueCreateAt(raw: unknown, fallback: number = Date.now()): number {
  if (raw == null || raw === "")
    return fallback;
  ...
  if (typeof raw === "object" && raw !== null) {
    const toNumber = (raw as Record<string, unknown>).toNumber;
    if (typeof toNumber === "function") {
      const ms = normalizeEpochMs((toNumber as () => number).call(raw));
      if (ms > 0)
        return ms;
    }
  }
...
export const A8_MATCH_MAX_FUTURE_MS: number = A8_MATCH_MAX_FUTURE_SEC * 1000;
export const A8_MATCH_LIST_MAX_FUTURE_MS: number = A8_MATCH_LIST_MAX_FUTURE_SEC * 1000;
