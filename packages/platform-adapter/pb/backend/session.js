...
/** ???? platforms.json???????? / import-platform 忱???????? PB_GATEWAY/PB_TOKEN ???????? */
export function loadPlatformsJsonSession() {
  const session = loadFromPlatformsJson();
  if (!session) {
    throw new Error(
      "??? PB ???????? npm run account:import-platform -- <???data> 忱?? gamebet_backend/data/esport/platforms.json",
    );
  }
  if (!buildAuthHeaders(session)) {
    throw new Error("PB token ??完??????? x-app-data / BrowserSessionId_* ?? JSON ?????");
  }
  return session;
}
...
/** PB token ??? `a` ??曳?base64 JSON??????? betCredit?????完??????????? */
