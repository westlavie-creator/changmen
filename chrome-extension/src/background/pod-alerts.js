...
/** @type {{ alerts: unknown[]; capturedAt: number; href: string; gridFound: boolean; sourceConnected: boolean }} */
let snapshot = {
  alerts: [],
  capturedAt: 0,
  href: "",
...
export function ingestPodAlertsMessage(message) {
  ...
  if (row.type !== "podAlertsSnapshot")
    return false;
  const body = /** @type {{ alerts?: unknown; capturedAt?: unknown; href?: unknown; gridFound?: unknown }} */ (message);
  snapshot = {
