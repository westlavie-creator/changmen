...
| 路径 | 说明 |
|------|------|
| `frontend/vendor/ui-bundle/index.js` | 第三方聚合前端打包产物（Vue 编译结果），**只读参考** |
| `frontend/console/` | `npm run patch:ui` 生成的 patch 版本，由浏览器加载 |

## 使用方式

- **浏览器**：访问 `http://localhost:3456/console/`，加载 patch 后的 bundle
- **Node**：禁止 `require` / import `vendor/ui-bundle/index.js`
- **插件**：加载 `frontend/extension/`（Chrome unpacked）
...
| 第三方 relay IP | WS 聚合，本项目用本地 `backend/proxy` + `/esport/ws/*` |

各平台采集仍直连**源站**（OB gateway、365ray 等）。

## 本地 API

- `POST /esport/Client_*`、`POST /esport/API_*` → `backend/esport-api/`
- `GET|POST /esport/pb/proxy?url=…` → `backend/proxy/pb_http_proxy.js`（PB 余额/下单/注单，凭证来自 `PB_*` / `platforms.json`）
- `GET /esport2/version.json` → 静态 stub（`backend/public/esport2/`）
- `WS /esport/ws/{OB,RAY,TF}` → `backend/proxy/`

默认账号：`admin` / `admin`（首次启动写入 `backend/data/esport/`）。
...
| OB demo 登录 URL → `/api/ob/demo-login` | 禁用/跳过 NMe、bQe 等浏览器采集器 |
...
2. 新平台接入在 `backend/platforms/<id>/` 实现；UI 契约按 bundle 内 `Client_*` / `API_*` 对齐。
3. 改 UI 行为时先查 `frontend/vendor/ui-bundle`，再决定是否值得 patch；默认只改地址类配置。
4. 禁止手改 `frontend/console/index.js`，改 patch 后执行 `npm run patch:ui`。
