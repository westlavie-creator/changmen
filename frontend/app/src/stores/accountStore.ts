...
async function accRefresh(this: ReturnType<typeof useAccountStore>, account: PlatformAccount) {
  ...
  try {
    ...
    account.balance = undefined;
    account.balanceError = normalizeBalanceError(e);
    return false;
  } finally {
...
function normalizeBalanceError(err: unknown): string {
  ...
  if (/failed to fetch|networkerror|network error|econnrefused|后端未连接/.test(lower)) {
    return "后端未连接，请先运行 npm run web（3456）";
  }
