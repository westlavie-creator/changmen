...
| 角色 | 组件 | 职责 |
|------|------|------|
| **客户端** | `gamebet_frontend`、Chrome 插件 | 连各博彩平台、采集比赛/赔率、下注；通过 `API_SaveMatch` / `API_SaveBet` 上报 |
| **服务端** | `gamebet_backend`、`gamebet_matcher`、Supabase | 接收上报、合并赛事（`client_matches`）、鉴权、账号/订单、HTTP 代理 |

开发时后端 `localhost:3560`（Windows 默认，见 `backend.bat`）/ `3456`（非 Windows）+ Vite `5174` 为本机联调地址；生产见 [PRODUCTION_DEPLOYMENT.md](./PRODUCTION_DEPLOYMENT.md)（同源 `/` + `/esport/*`）。
...
```bat
npm install                            # workspaces：packages/shared + backend + matcher
npm install --prefix gamebet_frontend
setup-dev-env.bat                      # 首次：从 .env.example 复制 gamebet_backend/.env
```

（仅改 backend 时也可 `npm install --prefix gamebet_backend`，但 matcher 依赖 `packages/shared/db` 需根目录 install 以 hoist `@supabase/supabase-js`。）

### Start (development)

HTTP 由 `gamebet_backend/server.js` 提供。**开发**：`dev.bat`（backend + Vite + matcher + Chrome 插件）；**生产**见 [PRODUCTION_DEPLOYMENT.md](./PRODUCTION_DEPLOYMENT.md)。

```bat
dev.bat                   # backend + Vite 5174 + matcher（推荐）
...
```bat
cd gamebet_backend && npm run web    # server.js
...
```bat
npm run app:build         # vue-tsc + vite build → gamebet_frontend/dist/
```
...
Frontend-only（在 `gamebet_frontend/`）：
...
├── gamebet_backend/      Node.js ESM，port 3560 (Win) / 3456
│   ├── server.js             HTTP 入口
...
├── gamebet_frontend/
│   ├── app/              Vue 3 + TypeScript + Vite (base /)
...
All A8 parity tracking lives under `gamebet_frontend/docs/`:
