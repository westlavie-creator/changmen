...
（仅改 backend 时也可 `npm install --prefix apps/backend`，但 matcher 依赖 `packages/shared/db` 需根目录 install 以 hoist `@supabase/supabase-js`。）

### Start (development)

HTTP 由 `apps/backend/server.js` 提供。**开发**：`dev.bat`（backend + Vite + matcher + Chrome 插件）；**生产**见 [PRODUCTION_DEPLOYMENT.md](./PRODUCTION_DEPLOYMENT.md)。

```bat
dev.bat                   # backend + Vite 5174 + matcher（推荐）
dev-web.bat               # 同 dev.bat（别名）
setup-dev-env.bat         # 首次：复制 apps/backend/.env
...
| `apps/matcher` | `packages/match-engine`、`packages/shared/*`、`packages/team-resolver` |
