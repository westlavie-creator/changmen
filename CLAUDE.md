...
| **服务端** | `apps/backend`、`apps/matcher`、RDS（PostgreSQL） | 接收上报、合并赛事（`client_matches`）、鉴权、账号/订单、HTTP 代理 |
...
| `packages/db/`（`@changmen/db`） | 数据层唯一入口：RDS / PostgreSQL（`index.js` → `impl_rds.js`） |
| `core/db/store.js` | 内存缓存（`_cache`）+ RDS 异步写；账号读写的主路径 |
| `core/account/order_store.js` | 订单读写，RDS `orders` 表，无本地副本 |

Auth: 自签 JWT（`users` + `profiles`）。凭证在 `apps/backend/.env`（`JWT_SECRET`、`DATABASE_URL` 或 `DATABASE_URL_PUBLIC` / `DATABASE_URL_INTERNAL`）。
