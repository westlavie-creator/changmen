...
import { collectRayGet } from "@/collectors/ray/http";
    ...
    try {
      ...
      for await (const msg of channel) {
        ...
        for (const row of data.odds) {
          ...
          const nextOdd = Number(row.odds) || 0;
          if (!id || !nextOdd || !odds.isOdds(PLATFORM, id)) continue;
          const cached = odds.getEntry(PLATFORM, id);
          odds.save(PLATFORM, {
            id,
            odds: nextOdd,
            isLock: row.status !== 1 ? true : (cached?.isLock ?? false),
            betId: String(row.odds_group_id ?? row.market_id ?? cached?.betId ?? ""),
            side: cached?.side,
            time: Date.now(),
          });
        }
