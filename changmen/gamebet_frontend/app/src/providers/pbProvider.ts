...
import { getPbLineId } from "@/collectors/pb/lineCache";
import { pbPluginPost } from "@/collectors/pb/pluginHttp";
import { toNumber } from "@/collectors/pb/core";
import { startPbRejectPoll, pbRejectStorageKey } from "@/providers/pbRejectPoll";
import type { PlatformProvider, VenueOrder, VenueOrderStatus } from "@/providers/types";
...
export const pbProvider: PlatformProvider = {
  ...
  async getOrders(account) {
    ...
    for (const filter of filters) {
      ...
      const data = await pbPluginPost<unknown[] | { error?: string }>(account, path, body, {
  ...
  async checkBet(account, option) {
    ...
    const rows = await pbPluginPost<PbOddsSelectionRow[]>(account, probePath, {
      oddsSelections: [
        {
          oddsFormat: 1,
          oddsId: itemId,
  ...
  async betting(account, option) {
    ...
    try {
      ...
      body = await pbPluginPost<PbBuyResponse>(account, path, option.data, {
        "content-type": "application/json; charset=UTF-8",
      });
