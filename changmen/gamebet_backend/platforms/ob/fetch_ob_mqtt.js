 ...
 * Flow: login → game/view baseline → subscribe MQTT topics → print updates.
 *
 * Usage:
 *   node scripts/platforms/ob/fetch_ob_mqtt.js --match 5385124130449945 --stage 1
 *   node scripts/platforms/ob/fetch_ob_mqtt.js --match ID --stage 0 --duration 120
 */

const mqtt = require("mqtt");
const Core = require("./ob_core.js");
const { login, fetchGameView } = require("./ob_session.js");
...
async function main() {
  ...
  if (!args.matchId) {
    console.error(
      "Usage: node scripts/platforms/ob/fetch_ob_mqtt.js --match MATCH_ID [--stage N] [--duration SEC] [--mqtt URL]"
    );
    process.exit(1);
  }
