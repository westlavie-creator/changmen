...
function atUtc(y: number, m: number, d: number, h = 12): number {
...
describe("month key helpers", () => {
  it("parses YYYY-MM and falls back on junk", () => {
    ...
    const fb = new Date(Date.UTC(2024, 0, 15));
  ...
  it("formats current month as YYYY-MM", () => {
    expect(currentMonthKey(new Date(Date.UTC(2026, 8, 12)))).toBe("2026-09");
  });
});

describe("aggregateBuilderFeesByDay", () => {
  it("fills every UTC day with zeros when empty", () => {
  ...
  it("sums builderFee / fee / volume and buy-sell split on the UTC day", () => {
    const rows = aggregateBuilderFeesByDay([
      {
        matchTime: atUtc(2026, 9, 2, 9),
        sizeUsdc: 10,
        feeUsdc: 0.4,
        builderFeeUsdc: 0.1,
        side: "BUY",
  ...
  it("drops trades outside the month and without matchTime", () => {
    const rows = aggregateBuilderFeesByDay([
      { matchTime: atUtc(2026, 8, 31), sizeUsdc: 99, feeUsdc: 1, builderFeeUsdc: 1, side: "BUY" },
      { matchTime: null, sizeUsdc: 99, feeUsdc: 1, builderFeeUsdc: 1, side: "BUY" },
      { matchTime: atUtc(2026, 9, 1, 0), sizeUsdc: 2, feeUsdc: 0.1, builderFeeUsdc: 0.01, side: "BUY" },
    ], "2026-09");
  ...
  it("utcDayKey uses UTC calendar", () => {
    expect(utcDayKey(Date.UTC(2026, 8, 12, 0, 0, 0, 0))).toBe("2026-09-12");
    expect(utcDayKey(Date.UTC(2026, 8, 12, 23, 59, 0, 0))).toBe("2026-09-12");
    expect(utcDayKey(Date.UTC(2026, 8, 11, 16, 0, 0, 0))).toBe("2026-09-11");
  });
});
...
describe("chart scale helpers", () => {
  it("maxSeriesValue reads the requested keys", () => {
    const rows = aggregateBuilderFeesByDay([
      { matchTime: atUtc(2026, 9, 1), sizeUsdc: 100, feeUsdc: 1, builderFeeUsdc: 0.2, side: "BUY" },
      { matchTime: atUtc(2026, 9, 2), sizeUsdc: 40, feeUsdc: 3, builderFeeUsdc: 0.5, side: "SELL" },
    ], "2026-09");
