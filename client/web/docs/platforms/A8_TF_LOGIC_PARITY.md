...
| 4 | 比赛列表 30s | `UBe` �?`o()` | `client/venue-adapter/tf/collect.ts` `TF_POLL_MS=30000` | 已实�?|
...
| 7 | 赔率 WS | `FBe`/`WBe` | `client/venue-adapter/tf/ws.ts` �?`relayWsUrl(/esport/ws/TF)` | 已实�?|
...
| 采集入口 | `client/venue-adapter/tf/collect.ts` |
| WS | `client/venue-adapter/tf/ws.ts` |
| HTTP 采集 | `client/venue-adapter/tf/http.ts` |
