...
浏览器 **直连** A8 聚合 WS（`ia/realtime.ts`，`Origin`/`auth.token` = `Client_GetCollectPlatform` 的 Gateway）。不经 `/esport/ws/IA` 本地 relay / Electron IPC。
...
`ia/transport.ts` — 对齐 A8 `Zn.get` / `Zn.post`：经 **Gamebet Chrome 扩展** 代发 `{gateway}/api/game/...`（header `token`）。无扩展时采集跳过（与 PB 相同门控）。

后端 `/esport/ia/proxy` 保留供脚本/调试，**前端采集不再使用**。

## 后端对照

`devtools/platform-probes/ia/ws.js` — WS 入口配置（探针 / relay）。
