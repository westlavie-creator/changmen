...
changmen 实现：`client/web` + `packages/venue-adapter/{platform}/` + `chrome-extension` + `@/chrome-plugin/bridge.ts`（A8 `Zn`）�?
最后更新：2026-06-11
...
```
浏览器托管页（脚本）          Chrome 插件（Zn�?├── packages/venue-adapter 采集     ├── GET/POST 跨域
...
| �?| 说明 |
|----|------|
| `packages/venue-adapter` 迁移 | 11 平台 collect/bet + registry |
| 插件协议 | `bridge.ts` = Zn |
...
| P2 | 文档：`A8_COMPARE_ALL_PLATFORMS` 路径�?`packages/venue-adapter` |
...
| A8 Socket | `packages/venue-adapter/shared/socket/hub.ts` |
| 平台实现 | `packages/venue-adapter/{ob,im,ray,...}/` |
