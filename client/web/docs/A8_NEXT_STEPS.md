...
| 项 | 状态 |
|----|------|
| 多运动（棒球/足球 + PM∥PF + N3 合并含足球让球/大小 + N3.5 实时赔率） | **维护态** — [ARB_MULTI_SPORT.md](../../../docs/ARB_MULTI_SPORT.md)；PF 非胜负盘供给以中超为主；**电竞实现面冻结**（`esport-freeze.json` + `check:esport-freeze`）；**不开 N4 下单** / Team UI / sport 下注；体育板不写 fo；足球 N4-detect Phase 0 抽检已记（中超同线可用） |
...
| 下一产品闸门 | 足球 **N4-detect**（只检测）→ 再要自动下单才开 **N4-bet**；棒球 moneyline / 足球让球+大小 — 见 [ARB_MULTI_SPORT.md](../../../docs/ARB_MULTI_SPORT.md)；或新场馆对齐另开 plan |
