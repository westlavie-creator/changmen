...
const { getAccountBalance, enrichAccountFromPlatformDefaults, parsePbTokenBalance } = require("./balance_provider.js");
...
function resolveStoredBalance(row) {
  ...
  if (String(row.provider || "").toUpperCase() === "PB" && row.token) {
    const cached = parsePbTokenBalance(row.token);
    if (cached?.balance != null) {
      return cached.balance * Math.max(1, Number(row.multiply) || 1);
    }
  }
...
/**
 * Client_GetData：数组 KV（ACCOUNT）直接返回数组，与 Io().loadAccounts 的 .map 对齐。
 */
function enrichAccountRowFromPlayer(row) {
  if (!row?.accountId) return row;
  const cached =
    String(row.provider || "").toUpperCase() === "PB" ? parsePbTokenBalance(row.token) : null;
  const balance = resolveStoredBalance(row);
  const usedCache =
    cached &&
    (row.balance == null || row.balance === 0) &&
    balance === cached.balance * Math.max(1, Number(row.multiply) || 1);
  if (balance == null || (balance === row.balance && !usedCache)) return row;
  return {
    ...row,
    balance,
    currency: usedCache ? cached.currency || row.currency : row.currency,
  };
}
