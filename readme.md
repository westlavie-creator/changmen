...
> **与 A8 的关系**：A8（`api.a8.to`）是第三方聚合平台，本项目借鉴其架构与采集思路在本机复刻实现。仓库内 `A8/index.js` 为从该站点拷贝的参考脚本，**仅供阅读逆向，禁止引用**。

## 仓库结构

| 目录 | 职责 |
|------|------|
| [`frontend/app/`](./frontend/app/) | **新控制台**（Vue 3 + Pinia，脱离 ui_bundle）；开发 `npm run app:dev`，生产 `/app/` |
| [`frontend/`](./frontend/) | A8 复刻：参考 bundle、patch 旧控制台 `/console/`、Chrome 扩展 |
| [`backend/`](./backend/) | 本地 Node：`esport-api`、WS 代理、可选 FeedHub、开发仪表盘 |
| [`A8js/`](./A8js/) | 逆向分析笔记 |

```bash
npm install --prefix backend
npm install --prefix frontend/app   # 新控制台依赖（首次）
...
## 本地复刻：多平台采集（frontend/app）

阶段 6（多平台采集）**已完成**：11 个平台均在 `frontend/app/src/collectors/` 注册，由 `collectors/index.ts` 按 `CollectConfig` 开关启停。

数据链路：

```text
采集器 → collectStore.saveMatch / saveBets
      → 后端 esport-api 合并
      → matchStore + oddsStore（对齐 A8 fo / Vg）
```
...
| RAY | `collectors/ray.ts` | SocketCluster + HTTP `/v2/match`、`/v2/odds` | `gateway` + `token`（API **强制** A8 写死凭证，见 `backend/shared/ray_a8_collect.js`） |
...
### 凭证存储：`backend/data/esport/platforms.json`
...
1. **插件导入**（推荐）  
   `cd backend && node account/account_cli.js import-platform <base64> --sync-store`
...
- 脚本：`backend/scripts/check-collect-platforms.js`
- 退出码 `0` = 所有**必需**凭证齐全；`1` = 仍有 TF/IA/IMT/SABA/Stake 等缺凭证
- Stake：`platforms.json` 使用字段 `accessToken`；`Client_GetCollectPlatform` 已映射为返回的 `Token`（`backend/esport-api/router.js`）
...
- `index.css`：前端样式文件。
- `plug/`：目标站使用的 Chrome 插件（现位于 `frontend/extension/`）。
