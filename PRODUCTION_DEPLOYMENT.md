...
│  predictfun-collector — 电竞 REST 采集已暂停（不随默认主栈启动） │
...
**电竞列表基线**：多数场馆仍为浏览器 `saveMatch` / `saveBet`；**Polymarket** 为 VPS collector 直写 `platform_*` + matcher → `Client_GetMatchs`。**PredictFun** 电竞 HTTP collector 已暂停（实时赔率仍靠浏览器 Market WS → `fo`；下注与体育 REST 不受影响）。**SXBet** 场馆已暂停（manifest `implementation: paused`）。
...
| Predict.fun HTTP 采集 | PM2：`changmen-predictfun-collector`（REST → `platform_*`）；**已暂停**，deploy 会 `pm2 delete` | 恢复：`pm2 start deploy/ecosystem.config.cjs --only changmen-predictfun-collector --update-env` |
...
```bash
cd changmen
# 推荐主栈（含 PM 电竞/体育 + PF Market WS hub + PM 电竞 discovery）
...
`changmen-predictfun-collector` 直连 `api.predict.fun`（`PREDICT_FUN_API_KEY`），写 `platform_matches` / `platform_bets` 与 `predictfun_market_index.json`。**当前已暂停**：deploy 会 `pm2 delete`，不随默认主栈启动。浏览器 Predict.fun 采集器**仅**经 `ws-forward` hub 订阅 orderbook → `fo`（hub 仍跑）。恢复采集：`pm2 start deploy/ecosystem.config.cjs --only changmen-predictfun-collector --update-env`。
