...
copy_from_old_server() {
  ...
  local remote_backend="${DEPLOY_REPO}/changmen/server/backend"
  if [ ! -d "$BACKEND" ] && [ -d "${DEPLOY_REPO}/server/backend" ]; then
    remote_backend="${DEPLOY_REPO}/server/backend"
  fi
...
setup_caddy() {
  ...
  local src="$CHANGMEN/scripts/Caddyfile"
  [ -f "$src" ] || src="$GIT_ROOT/changmen/scripts/Caddyfile"
  [ -f "$src" ] || { warn "Caddyfile not found"; return 0; }
  local patched="/tmp/changmen-Caddyfile.$$"
  sed "s|root \\* /root/gamebet/changmen/client/web/dist|root * $DIST_DIR|g" "$src" > "$patched"
  export CADDY_SRC="$patched"
  bash "$CHANGMEN/scripts/setup-caddy-remote.sh"
  rm -f "$patched"
}
...
main() {
  ...
  if need_cmd pm2; then
    if pm2 describe "$PM2_WEB" >/dev/null 2>&1; then
      log "pm2 restart"
      pm2 restart "$PM2_WEB" "$PM2_PM_SPORTS" --update-env || pm2 restart ecosystem.config.cjs --update-env
    else
      log "pm2 start"
