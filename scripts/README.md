...
| **Vite 前端** | `npm run app:dev` 或 `BAT\dev.bat` | 开发时 `5174/`，API 经 proxy 到后端 |
| **Chrome 插件** | `client/chrome-extension` | PB/Stake 代发、凭证采集、v4 桥接 |
| **Matcher** | `npm run matcher:loop` | 合并 `client_matches` |

`client/web` 已纳入 npm workspaces；根目录 `npm install` 会安装前端依赖，无需单独 `app:install`（该命令现为 `npm install` 别名）。
...
| `npm run app:install`（单独装 web） | → 根目录 `npm install`（workspaces 含 `client/web`） |
