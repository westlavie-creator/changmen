...
const cm = await pool.query(
  `SELECT id, title, game, start_time, bo, round, list_status, built_at, matchs, bets
...
if (!cm.rows.length) {
  ...
  const hidden = Number(row.list_status) === CLIENT_MATCH_LIST_HIDDEN;
  ...
  console.log("  list_status: ", row.list_status, hidden ? "(-1 隐藏，Web 不返回)" : "(0 可见)");
  ...
  console.log(
    hidden ? "\n❌ Web 不可见：list_status = -1（isClientMatchEnded）" : "\n✅ Web 应可见（list_status ≠ -1）",
  );
...
const vis = await pool.query(
  `SELECT COUNT(*)::int AS c FROM client_matches WHERE list_status IS DISTINCT FROM $1`,
...
const ferno = await pool.query(
  `SELECT id, title, list_status, matchs, built_at
   FROM client_matches
...
if (ferno.rows.length) {
  ...
  for (const r of ferno.rows) {
    console.log(`  #${r.id} list_status=${r.list_status} | ${r.title} | matchs=${JSON.stringify(r.matchs)}`);
  }
}

const bySrc = await pool.query(
  `SELECT id, title, list_status, matchs
   FROM client_matches
...
if (bySrc.rows.length) {
  ...
  for (const r of bySrc.rows) {
    console.log(`  #${r.id} list_status=${r.list_status} | ${r.title}`);
  }
}
