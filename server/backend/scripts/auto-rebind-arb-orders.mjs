...
function isUnboundOrder(row) {
  ...
  if (!Number.isFinite(link) || !Number.isFinite(ca) || ca <= 0) return false;
  if (link === ca - 1) return true;
  if (isCreateAtPlaceholderLink(link, ca)) return true;
  if (isHashLink(link)) return true;
  if (link >= ARB_LINK_MIN && link > ca) return true;
  return false;
}
...
function buildAttemptsFromLogs(logs) {
  ...
  for (const row of logs) {
    if (!String(row.title).includes("请求盘口数据")) continue;
  ...
  for (let i = 0; i < checks.length; i += 1) {
    ...
    for (let j = i + 1; j < checks.length; j += 1) {
      ...
      if (other.match !== seed.match) continue;
      if (Math.abs(other.at - seed.at) > 5000) continue;
      if (group.some((g) => g.provider === other.provider)) continue;
      group.push(other);
      used.add(j);
    }
...
function pickOrdersForAttempt(attempt, orders, usedOrderIds) {
  ...
  for (const row of orders) {
    if (usedOrderIds.has(String(row.order_id))) continue;
    if (!isUnboundOrder(row)) continue;
    if (!attempt.providers.has(String(row.provider))) continue;
    if (normMatch(row.match) !== attempt.match) continue;
    const ca = Number(row.create_at);
    if (ca < windowStart || ca > windowEnd) continue;
...
function heuristicPairs(orders, usedOrderIds) {
  ...
  for (let i = 0; i < remaining.length; i += 1) {
    const a = remaining[i];
    if (usedOrderIds.has(String(a.order_id))) continue;

    let best = null;
    let bestDelta = Infinity;
    for (let j = i + 1; j < remaining.length; j += 1) {
      const b = remaining[j];
      if (usedOrderIds.has(String(b.order_id))) continue;
      if (String(a.provider) === String(b.provider)) continue;
      if (normMatch(a.match) !== normMatch(b.match)) continue;
      const delta = Math.abs(Number(a.create_at) - Number(b.create_at));
      if (delta > 90_000) break;
