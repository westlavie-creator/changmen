#!/usr/bin/env node
/**
 * GB12 OB：删除 player 36（旧九游）上与 player 23 重复的订单副本。
 *
 * Usage:
 *   node scripts/cleanup-gb12-ob-player36-dupes.mjs --dry-run
 *   node scripts/cleanup-gb12-ob-player36-dupes.mjs
 */
import { loadChangmenEnv } from "@changmen/storage/load_env.js";

const dryRun = process.argv.includes("--dry-run");
const USER_NAME = "GB12";
const KEEP_PLAYER = 23;
const DROP_PLAYER = 36;
const PROVIDER = "OB";
const BACKUP_TABLE = "orders_dup_backup_gb12_ob36_20260712";
...
const orphan36 = await pool.query(
...
console.log(`player ${DROP_PLAYER} OB orders without player ${KEEP_PLAYER} counterpart: ${orphan36.rows[0].n}`);

for (const row of preview.rows) {
  console.log(
    `  id=${row.id} order_id=${row.order_id} link=${row.link ?? "-"} status=${row.status} match=${row.match ?? "-"}`,
  );
}
