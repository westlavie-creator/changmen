#!/usr/bin/env node
/**
 * GB11：硬删除全部软删 player 及其 orders（先备份）。
 *
 *   node scripts/purge-gb11-soft-players.mjs --dry-run
 *   node scripts/purge-gb11-soft-players.mjs --execute
 */
import { loadChangmenEnv } from "@changmen/storage/load_env.js";

const dryRun = !process.argv.includes("--execute");
const USER_NAME = "GB11";
const BACKUP_PLAYERS = "players_purge_backup_gb11_soft_all_20260712";
const BACKUP_ORDERS = "orders_purge_backup_gb11_soft_all_20260712";
...
if (dryRun) {
  ...
  console.log("执行: node scripts/purge-gb11-soft-players.mjs --execute");
  await pool.end();
  process.exit(0);
}
