...
VPS **守护进程级采集** 归集目录（浏览器采集仍在 `packages/venue-adapter/`）。
...
| `@changmen/polymarket-football-collector` | [polymarket-football/](polymarket-football/README.md) | PM 足球 Gamma+/prices → 不能直连用户的 fallback 快照 |
...
根目录 `npm run pm-sports` / `polymarket-collector` / `pm-football-collector` / `predictfun-collector` / `sxbet-collector`；PM2 见 `deploy/ecosystem.config.cjs`。
...
| parse（A8 馆） | 浏览器 `packages/venue-adapter/{platform}/` 仍为采集权威 |
