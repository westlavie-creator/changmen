...
> **适用范围**：主要描述 `node/core.js`（CLI / `applyMqttPayload`）。**生产 UI** 使用 `frontend/mqtt.ts`，在 `market.statusUpdate` 上对齐 A8（无 `status` 时一律锁盘），可能与本文 §6 表格中 `core.js` 行为不同。HTTP 锁盘公式以 `../shared/save_bets.ts` 为准。
...
实现位置：`node/core.js` → `applyMqttPayload`（CLI）。浏览器见 `frontend/mqtt.ts`。
...
| 字段 | 来源 | UI |
|------|------|-----|
| `oddsStore` / fo `locked` | `frontend/mqtt.ts` + HTTP `markets.ts` | 套利列表赔率与锁态 |
...
3. **组合盘 / 非「获胜」主盘**：UI 主盘选取见 `frontend/markets.ts` / `pickWinMarket`。
