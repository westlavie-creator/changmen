...
`requirePlatform("OB", "node", "session.js")` → `packages/platform-node/ob/session.js`。
...
# → apps/backend/platform_node
```

与 `platform-adapter` 的 sync 成对使用（`apps/backend/platform_adapter`）。