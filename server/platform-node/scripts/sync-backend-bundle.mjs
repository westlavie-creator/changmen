#!/usr/bin/env node
/**
 * 将 server/platform-node 同步到 server/backend/platform_node（瘦包部署）。
 */
...
export function syncPlatformNodeBundle(options = {}) {
  ...
  if (!fs.existsSync(path.join(src, "ob", "session.js"))) {
    throw new Error(`platform-node source missing ob/session.js: ${src}`);
  }
...
if (isMain) {
  ...
  console.log(`[sync-platform-node] ${dst}`);
}
