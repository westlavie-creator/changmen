...
浏览器采集：`client/platform-adapter/pb/frontend` �?`API_SaveMatch` / `API_SaveBet`；余�?下单通过 **Chrome 插件** 代发（需安装插件并在平博页保持登录）�?
...
| `PB_SYNC_MS` | （历�?Feed）轮询间隔；浏览器采集间隔见 `frontend/collect.ts` |
...
| 赔率采集 | `bQ` �?euro/odds | 浏览�?`client/platform-adapter/pb/frontend` |
