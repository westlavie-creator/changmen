/**
 * Polymarket 体育 MARKET WebSocket Hub — 与电竞 `pm_market_hub.js` 完全独立副本。
 * 浏览器连 /esport/ws-forward/PM-SPORT-MARKET；勿改电竞 core/pm_market_hub.js。
...
export const PM_SPORT_MARKET_HUB_PATH = "/esport/ws-forward/PM-SPORT-MARKET";
const HUB_ID = "PM-SPORT-MARKET";
 ...
 */
export function attachPmSportMarketHub(httpServer, opts = {}) {
  if (wss)
    return;
  ...
  httpServer.on("upgrade", (request, socket, head) => {
    ...
    if (pathname !== PM_SPORT_MARKET_HUB_PATH && !pathname.startsWith(`${PM_SPORT_MARKET_HUB_PATH}/`))
      return;
...
export function getPmSportMarketHubStatus() {
  const now = Date.now();
...
export function closePmSportMarketHub() {
  stopPendingFlushTimer();
  clearUpstreamReconnectTimer();
  stopHubBackpressure?.();
...
/** @internal 测试重置 */
export function resetPmSportMarketHubForTests() {
