...
const usedSymbols = [
  "authSignIn", "authSignOut", "authGetUser", "authRefreshToken", "isAuthConfigured",
  "fetchProfiles", "fetchProfileById", "writeProfile", "insertProfile", "writeAccounts",
  "fetchProfilesAdmin", "updateUserName", "updateUserIsAdmin", "writeUserMetadata",
  "upsertOrders", "setOrdersInsertedHook", "setOrdersBoundHook", "updateOrderBind",
  "fetchOrdersByDate", "fetchOrdersByPlayer", "fetchOrdersByPlayerAll",
  "fetchOrdersByLink", "fetchOrderByOrderId", "fetchUserByName", "fetchUserById",
  "fetchOrdersAdminStats", "fetchOrdersAdminPage", "deleteOrdersByIds",
  "fetchOrdersAdminAll", "fetchOrdersForMonthAggregate", "fetchOrdersForProfitAggregate",
  ...
  "isPbHashOrder", "isHashLink", "placeholderLinkFromCreateAt", "placeholderLinkFromInsertAt",
  "backendBindLinkFromCreateAt",
  "isInsertTimePlaceholderLink",
  "CLIENT_ORDER_LIST_SQL", "isClientOrderListVisible", "orderVisibleSqlAnd",
  "CHANGMEN_ORDER_LIST_SQL", "orderChangmenBetSqlAnd", "matchChangmenBetFromLogs",
  "betLogWindowForOrders",
  "isCreateAtPlaceholderLink", "isArbBindLink", "shouldFireOrderBoundHook",
  ...
  "fetchCanonicalTeamExact", "findCanonicalTeamByNormalizedName", "upsertTeamPlatformMaps",
...
for (const k of implKeys) {
  if (!(k in db)) fail(`index 缺少 impl 导出: ${k}`);
}
for (const k of implKeys) {
...
if (failed === 0) ok(`${implKeys.length} 个 impl 符号均在 @changmen/db 可用`);

console.log("\n=== 3. 业务侧常用符号 ===");
for (const sym of usedSymbols) {
  if (!(sym in db)) fail(`@changmen/db 缺少: ${sym}`);
  else if (typeof db[sym] === "undefined") fail(`${sym} 为 undefined`);
}
