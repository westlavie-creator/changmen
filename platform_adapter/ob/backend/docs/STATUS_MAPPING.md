...
- [README.md](./README.md) — **登录 URL 等配置**（`OB_LOGIN_URL`）
- `ob_core.js` — `describeMatchStatus` / `describeStageStatus` / `describeMarketStatus`
- `ob_feed.js` — `sync()` 拉 index+view+getTimer，MQTT 增量合并
- `fetch_ob_mqtt.js` — 调试 MQTT 原始事件
...
选盘（取哪一条「获胜」主盘）见 [MARKETS.md](../../MARKETS.md) → OB `match_winner`。
...
| MQTT `market.*` / `odd.*` | 增量 `locked`（Feed 目前汇总到主盘 `winMarketStatus`） |
...
1. 在 `platforms/<id>/core.js` 实现各自的 `describeMatchStatus` / `describeMarketStatus`
2. Feed snapshot **统一输出** `matchStatus` / `stageStatus` / `marketStatus` 的 `code`+`label`
3. Dashboard 只读统一语义，不读 OB 的 `is_live=2` 等原始值
