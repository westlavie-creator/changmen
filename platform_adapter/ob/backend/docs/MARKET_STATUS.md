...
相关代码：`ob_core.js`（`isMarketLocked`、`describeMarketStatus`）  
探测脚本：`node scripts/platforms/ob/probe_market_status.js`
...
与 A8 采集端、浏览器插件逻辑一致（规则见 [`MARKETS.md`](../../MARKETS.md) → OB `openWhen`）：
...
实现位置：`ob_core.js` → `applyMqttPayload`。

**注意：** MQTT 只更新内存中的 `currentOdds[].locked`；Feed 层通过 `winHomeId` 关联到 UI 的 `winLocked`（仅「获胜」主盘）。
...
```bash
# 单场盘口 + status/visible/suspended/locked
node scripts/platforms/ob/fetch_ob_view.js --match <matchId> --stage 0

# 扫描多场比赛的状态组合分布
node scripts/platforms/ob/probe_market_status.js

# 监听 MQTT 锁盘/赔率事件
node scripts/platforms/ob/fetch_ob_mqtt.js --match <matchId> --stage 0 --duration 120
```
...
3. **组合盘 / 非「获胜」主盘**：Dashboard 主要展示 `pickWinMarket` 选出的主盘 `winLocked`。
