...
describe("buildObSaveBetRowsFromViewBlocks", () => {
  test("CS2 Map1: 单局-获胜 included, 手枪局 excluded", () => {
    const blocks = [
      obBlock({
        id: "5561517900269057",
        round: 1,
  ...
  test("hidden or settled blocks are skipped", () => {
    ...
    expect(isObBlockCollectable(block, "[全场]-全局-获胜", BET_RE)).toBe(false);
    expect(buildObSaveBetRowsFromViewBlocks([block], "m1", TEAMS, BET_RE)).toEqual([]);
  });
});
